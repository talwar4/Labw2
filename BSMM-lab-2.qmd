...
View(artist_missing_year_count)
...
View(gender_grouped) # display the gender grouped table
```
...
```{r}
install.packages("readr") # Install packages for reading the csv file#
library(dplyr)
library(readr)
